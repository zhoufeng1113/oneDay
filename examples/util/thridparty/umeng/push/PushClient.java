...
public class PushClient {
	...
	public boolean send(UmengNotification msg) throws Exception {
        ...
        System.out.println("UmengNotification->Post Body : " + msg.getPostBody());
        System.out.println("UmengNotification->Response Code : " + status);
        ...
        System.out.println("UmengNotification->" + result.toString());
        return true;
    }
...
//		if (vo.getDevice().length() > 44) {
		if(!vo.isAndroid()){
			IOSUnicast unicast = new IOSUnicast(ThirdPart.umeng_ios_appkey, ThirdPart.umeng_ios_appMasterSecret);
			unicast.setDeviceToken(vo.getDevice());
			unicast.setAlert(vo.getContent());
			unicast.setBadge(0);
			unicast.setSound("default");
			unicast.setProductionMode(!ThirdPart.push_debug);
		...
		//android
			...
			AndroidUnicast unicast = new AndroidUnicast(ThirdPart.umeng_android_appkey,ThirdPart.umeng_android_appMasterSecret);
			unicast.setDeviceToken(vo.getDevice());
	...
	public void sendMsgListcast(PushMassageVo vo,String tokens,String client) throws Exception {
		...
		if (client.equals(CommArray.EnumClient._iphone.getCode())) {
			IOSListcast listcast = new IOSListcast(ThirdPart.umeng_ios_appkey, ThirdPart.umeng_ios_appMasterSecret);
			listcast.setDeviceTokens(tokens);
			listcast.setAlert(vo.getContent());
			listcast.setBadge(0);
			listcast.setSound("default");
			listcast.setProductionMode(!ThirdPart.push_debug);
			// Set customized fields
		...
		//android
			...
			AndroidListcast listcast = new AndroidListcast(ThirdPart.umeng_android_appkey,ThirdPart.umeng_android_appMasterSecret);
			listcast.setDeviceTokens(tokens);
